...
bool PinyinFinder::init(const std::string& singleCharacterDictPath,
  ...
  while (fgets(line, sizeof(line) - 1, fp)) {
    ...
    utils::SplitString(pinyin.c_str(), pinyin.size(), ',', &ss);
    if (!ss.empty()) {
...
//   LOG_INFO << "total pinyin character count: " << tn << std::endl;
  ...
  while (fgets(line, sizeof(line) - 1, fp)) {
    ...
    UnicodeStr ustr;
    utils::u8tou16(word.c_str(), word.size(), ustr);
...
void PinyinFinder::find_best_pinyin(const std::string& phrasestr, std::vector<std::string>& pinyins) {
  UnicodeStr phrase;
  utils::u8tou16(phrasestr.c_str(), phrasestr.size(), phrase);
  int n = phrase.size();
  if (n == 0) return;
  ...
  while (i <= j) {
    ...
    if(it == word_pinyin_dict_.end()){
      std::string tstr;
      utils::u16tou8(sub.data(), sub.size(), tstr);
      pinyins.emplace_back(tstr);
    } else {
      std::vector<std::string> tmps;
      utils::SplitString(it->second.c_str(), it->second.size(), ' ', &tmps);
      for(auto& str: tmps){
