...
public:
    ...
    bool init(const TTSFrontendConfig& config) {
        en_g2p_ = std::make_unique<utils::EnEspeakG2P>();
        zh_g2p_ = std::make_unique<utils::ZhEspeakG2P>();
    ...
    std::vector<int> run(const std::string& input_text, const std::string& language, const std::map<std::string, int>& vocab, int& err) {
        ...
        std::string phonemes;
        if (language == "en")
            phonemes = en_g2p_->run(normalized_text, err);
        else if (language == "zh")
            phonemes = zh_g2p_->run(normalized_text, err);
        else
            phonemes = latin_g2p_->run(normalized_text, err);
...
private:
    ...
    std::unique_ptr<utils::G2P> en_g2p_, zh_g2p_, latin_g2p_;
};